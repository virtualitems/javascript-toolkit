...
is.builtBy = (constructor, value) =>
  value !== undefined && value !== null && value.constructor === constructor
is.not.builtBy = (constructor, value) =>
  value === undefined || value === null || value.constructor !== constructor
...
is.not.esModule = (value) =>
  value instanceof Object === false || value.__esModule !== true
...
export function allAre(proto, ...objects) {
  ...
  return objects.every((obj) =>
    obj === undefined || obj === null ? false : obj.constructor === proto
  )
}
...
export function ensure(...values) {
  ...
  if (firstElement === undefined) {
    for (let i = 1; i < length; i += 1) if (values[i] !== undefined) return false
    return true
  }

  if (firstElement === null) {
    for (let i = 1; i < length; i += 1) if (values[i] !== null) return false
    return true
  }

  if (Object.is(firstElement, NaN)) {
    for (let i = 1; i < length; i += 1)
      if (Object.is(values[i], NaN) === false) return false
    return true
  }

  for (let i = 1; i < length; i += 1)
    if (values[i]?.constructor !== firstElement.constructor) return false
